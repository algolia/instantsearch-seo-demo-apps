...
const Ratings = ({ currentRefinement, refine, createURL, count }: any) => {
  const ratings = new Array(4).fill(null).map((_, ratingIndex) => {
    ...
    const itemsCount = count
      .filter(
        (countObj: { count: number; value: string }) =>
          value <= parseInt(countObj.value, 10)
      )
      .map((countObj: { count: number; value: string }) => countObj.count)
      .reduce((sum: number, currentCount: number) => sum + currentCount, 0);
